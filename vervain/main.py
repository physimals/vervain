...
def main():
    ...
    adv.add_option("--logcbf", action="store_true", help="Use log transform internally for CBF inference", default=False)
    ...
    adv.add_option("--baseline-points", help="Number of baseline points used to determine S0 for signal->concentration conversion", type="int", default=1)
...
def get_mask(wsp):
    ...
    if wsp.mask is None:
        ...
        wsp.log.write("  - Mask generated by running BET on time-averaged DSC data\n")
    else:
...
def get_aifconc(wsp):
    ...
    """
    if wsp.aifconc:
        wsp.log.write("  - AIF supplied  as concentration-time curve\n")
        wsp.aif_conc = wsp.aif
    else:
        wsp.log.write("  - AIF supplied  as DSC signal curve\n")
...
def sig_to_conc(wsp, img):
    ...
    """
    wsp.log.write("  - Converting DSC data to concentration using %i baseline points" % wsp.baseline_points)
    sig0 = np.mean(img.data[..., :wsp.baseline_points], axis=-1)
    while sig0.ndim < 4:
...
def no_negatives(wsp, name):
    """
    Threshold an image so it contains no negatives
    """
    img = getattr(wsp, name)
    data = img.data
    data[data < 0] = 0
    setattr(wsp, name, Image(data, header=img.header))
...
def do_vascular_model(wsp):
    ...
    if wsp.logcbf:
        wsp.log.write("  - Using log-transform on CBF")
        options["PSP_byname1"] = "cbf"
        options["PSP_byname1_transform"] = "L"
        options["PSP_byname1_prec"] = 0.0001

    if wsp.spatial == "standard":
        options["param-spatial-priors"] = "MN+"
    elif wsp.spatial == "full":
        options["param-spatial-priors"] = "MMN+"
    elif wsp.spatial == "none":
        options["param-spatial-priors"] = "N+"
    ...
    wsp.output.sub("vm")
    wsp.output.vm.rcbf = wsp.vm.mean_cbf
    wsp.output.vm.mtt = wsp.vm.mean_transitm
    wsp.output.vm.lam = wsp.vm.mean_lambda
    no_negatives(wsp.output.vm, "rcbf")

    # Macrovascular component
    if wsp.mv:
        wsp.log.write("\nBegin VM + MV analysis\n")
        wsp.sub("vm_mv")
        ...
        wsp.init_cbv = Image(cbv, header=wsp.dscdata_conc.header)

        cbf = mvntool(wsp.vm.finalMVN, 2, output=fsl.LOAD, mask=wsp.mask, val="", log=wsp.fsllog)["output"]
        init_cbf = cbf.data / 10.0
        init_cbf[~np.isfinite(init_cbf)] = 0
        wsp.init_cbf = Image(init_cbf, header=cbf.header)

        # Insert initialization for CBF, ABV and artdelay into the MVN
        mvn = mvntool(wsp.vm.finalMVN, 2, output=fsl.LOAD, write=True, valim=wsp.init_cbf, mask=wsp.mask, log=wsp.fsllog)["output"]
        mvn = mvntool(mvn, 4, output=fsl.LOAD, new=True, valim=wsp.init_cbv, var=0.1, mask=wsp.mask, log=wsp.fsllog)["output"]
        wsp.vm_mv.init_mvn = mvntool(mvn, 5, output=fsl.LOAD, new=True, val=5, var=1, mask=wsp.mask, log=wsp.fsllog)["output"]
        
        options["continue-from-mvn"] = wsp.vm_mv.init_mvn
        if wsp.spatial == "none":
            options["param-spatial-priors"] = "NNNANNN"
        else:
            options["param-spatial-priors"] = "MMNANNN"
        options["inferart"] = True
        if wsp.sigadd:
            wsp.log.write("  - Combining tissue and arterial results as sum of signals")
            option["artoption"] = True
        else:
            wsp.log.write("  - Combining tissue and arterial results as sum of concentrations")
        ...
        # Copy results to output directory
        wsp.output.sub("vm_mv")
        wsp.output.vm_mv.rcbf = wsp.vm_mv.mean_cbf
        wsp.output.vm_mv.mtt = wsp.vm_mv.mean_transitm
        wsp.output.vm_mv.lam = wsp.vm_mv.mean_lambda
        wsp.output.vm_mv.rabv = wsp.vm_mv.mean_abv
        no_negatives(wsp.output.vm_mv, "rcbf")
        no_negatives(wsp.output.vm_mv, "rabv")
